...
try:
    from puppeteer_core import launch, connect
except ImportError:
...
class PuppeteerPage:
    def __init__(self, ws_url=None, open_browser=False):
        ...
        self.browser = self.loop.run_until_complete(connect(browserWSEndpoint=ws_url))
        self.page = self.loop.run_until_complete(self.browser.newPage())

        self.graphql_responses = []

        self.loop.run_until_complete(self._setup_response_handler())
            ...
            if "/api/graphql/" in url or "/graphql/" in url:
                try:
                    ...
                    if asyncio.iscoroutine(text):
                        text = self.loop.run_until_complete(text)
                    self.graphql_responses.append({"url": url, "text": text})
                except Exception:
                    pass
    ...
    def navigate(self, url):
        self.loop.run_until_complete(
            self.page.goto(url, waitUntil="domcontentloaded", timeout=90000)
        )
    ...
    def click_element(self, selector):
        self.loop.run_until_complete(self.page.click(selector))

    def wait_for_selector(self, selector, timeout=30000):
        self.loop.run_until_complete(
            self.page.waitForSelector(selector, timeout=timeout)
        )

    def close(self):
        self.loop.run_until_complete(self.page.close())
        self.loop.run_until_complete(self.browser.disconnect())
        self.loop.close()


class PuppeteerRequestsParser:
...
class FacebookPuppeteerScraper:
    def __init__(self, ws_url=None, open_browser=False):
        self.page = PuppeteerPage(ws_url=ws_url, open_browser=open_browser)
        self.requests_parser = PuppeteerRequestsParser(self.page)
        self.post_id_list = []
